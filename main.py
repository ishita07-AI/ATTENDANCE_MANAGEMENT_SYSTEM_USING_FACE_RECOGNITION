...
class AttendanceSystem:
    ...
    def update_student_details_csv(self, csv_file_path):
        ...
        if not os.path.exists(csv_file_path):
            with open(csv_file_path, mode="w", newline="") as file:
                ...
                writer.writerow(["Enrollment", "Name", "Timestamp"])  # Add headers
    ...
    def load_registered_students(self):
        try:
            csv_file_path = "./StudentDetails/StudentDetails.csv"
            if not os.path.exists(csv_file_path):
                tk.Label(self.admin_window, text="No student details file found.", font=('times', 14, 'bold')).pack(pady=10)
                return

            student_data = pd.read_csv(csv_file_path, header=None, names=["Enrollment", "Name", "Timestamp"])
            if student_data.empty:
                tk.Label(self.admin_window, text="No registered students found.", font=('times', 14)).pack(pady=10)
                return

            for idx, student in student_data.iterrows():
                tk.Label(
                    self.admin_window,
                    text=f"ID: {student['Enrollment']} - Name: {student['Name']} - Timestamp: {student['Timestamp']}",
                    font=('times', 12)
                ).pack(pady=5)
        except Exception as e:
            tk.Label(self.admin_window, text=f"Error: {e}", font=('times', 14, 'bold')).pack(pady=10)
